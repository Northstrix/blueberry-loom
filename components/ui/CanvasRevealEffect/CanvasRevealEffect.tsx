...
export const CanvasRevealEffect: React.FC<CanvasRevealEffectProps> = ({
  ...
  // Stylish purple-blue color palette (RGB values)
  const colors = [
    [138, 43, 226], // Purple
    [65, 105, 225], // Blue
    [0, 191, 255],  // Sky blue (lighter, for accent)
  ];
  ...
  return (
    <div
      className={cn(
        ...
        containerClassName,
        replaceBackground ? "bg-transparent" : "bg-[var(--background)]"
      )}
    >
...
const ShaderMaterialComponent: React.FC<{ source: string; uniforms: Uniforms; maxFps?: number }> = ({
  source,
  uniforms,
  maxFps = 60,
}) => {
