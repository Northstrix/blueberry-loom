...
export default function FormBuilderWrapper({
    ...
    if (mode === "edit" && formId && isAuthenticated) {
      ...
      (async () => {
        try {
          ...
          const decodedTemplate = base64ToUint8Array(retrievedForm.encryptedFormTemplate);
  ...
  }, [mode, formId, isAuthenticated, userEmail, masterKey, iterations, extractForm, t, isRtl]);
  ...
  const handleSaveFormNew = async (plaintextFormTemplate: string) => {
    try {
      ...
      const paddedFormTemplateStringAsUint8Array = padToMultipleAsUint8Array(plaintextFormTemplate);
      const encryptedFormTemplate = await silentlyEncryptDataWithTwoCiphersCBCnoPadding(
  ...
  const handleDiscard = async () => {
    const message = `
      <p style="margin-bottom: 10px;" dir="${isRtl ? 'rtl' : 'ltr'}">${t('confirm_form_discard')}</p>
